...
fit_and_get_r_squared <- function(observed_data, observed_y, simulated_y) {
  # Create a new data frame with the observed predictor and simulated response
  combined_data <- data.frame(observed_y, simulated_y)
  
  # Remove observations with NA values
  combined_data <- na.omit(combined_data)
  
  # Fit the model of observed vs. simulated data
...
# Function to simulate datasets and collect R-squared values
simulate_r_squared <- function(observed_data, formula, n_simulations = 1000) {
  # Fit the initial model
  mod <- lm(formula, data = observed_data)
  
  # Generate simulated response variables using map()
  ...
  # Collect R-squared values using map_dbl()
...
# Assuming your observed data is in a data frame called 'data_by_country'
# and you have a formula specifying the relationship, e.g., y ~ x

formula <- happiness ~ corruption

# Generate 1000 simulated datasets and collect R-squared values
r_squared_values <- simulate_r_squared(data_by_country, formula, n_simulations = 1000)

# Plot the distribution of the R-squared values
